...
#define frontLeftTrigPin 2
#define frontLeftEchoPin 3

#define frontRightTrigPin 1
#define frontRightEchoPin 0

#define reverseTrigPin 11
#define reverseEchoPin 10  
...
// set up sonar
 ...
 pinMode(trigPin, OUTPUT);
  ...
  pinMode(reverseTrigPin, OUTPUT);
  pinMode(reverseEchoPin, INPUT);
...
// BACK SONAR
  ...
  digitalWrite(reverseTrigPin, LOW); 
  delayMicroseconds(2);
  digitalWrite(reverseTrigPin, HIGH);
  delayMicroseconds(10);
  digitalWrite(reverseTrigPin, LOW);
  durationBack = pulseIn(reverseEchoPin, HIGH);
  distanceBack = (durationBack/2) / 29.1;  
  
  // FRONT RIGHT SONAR
  
  long durationFrontRight, distanceFrontRight;
  digitalWrite(frontRightTrigPin, LOW); 
  delayMicroseconds(2);
  digitalWrite(frontRightTrigPin, HIGH);
  delayMicroseconds(10);
  digitalWrite(frontRightTrigPin, LOW);
  durationFrontRight = pulseIn(frontRightEchoPin, HIGH);
  distanceFrontRight = (durationFrontRight/2) / 29.1;   
  
// FRONT LEFT SONAR
  
  long durationFrontLeft, distanceFrontLeft;
  digitalWrite(frontLeftTrigPin, LOW); 
  delayMicroseconds(2);
  digitalWrite(frontLeftTrigPin, HIGH);
  delayMicroseconds(10);
  digitalWrite(frontLeftTrigPin, LOW);
  durationFrontLeft = pulseIn(frontRightEchoPin, HIGH);
  distanceFrontLeft = (durationFrontLeft/2) / 29.1; 
...
// *******************************************
    ...
    String jsonString = "{\"front\":\"";
    
    jsonString += distance;
    jsonString +="\",\"back\":\"";
    jsonString += distanceBack;
    jsonString +="\",\"right\":\"";
    jsonString += distanceRight;
    jsonString +="\",\"left\":\"";
    jsonString += distanceLeft;
  
      jsonString +="\",\"front-right\":\"";
    jsonString += distanceFrontRight;
    jsonString +="\",\"front-left\":\"";
    jsonString += distanceFrontLeft;
